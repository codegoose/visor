...
static std::optional<std::string> _sc_bootstrap(std::function<std::optional<std::string>(GLFWwindow *, ImGuiContext *)> success_cb) {
    ...
    const auto glfw_window = glfwCreateWindow(initial_framebuffer_size.x, initial_framebuffer_size.y, fmt::format("{} v{} (Development)", SC_APP_NAME, SC_APP_VER).data(), nullptr, nullptr);
